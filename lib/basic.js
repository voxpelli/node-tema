...
const recursiveReaddir = util.promisify(require('recursive-readdir'));
...
Tema.prototype.renderTemplate = function (options) {
  ...
  if (_.isFunction(options.toRender)) {
    return util.promisify(options.toRender.bind(this))(variables);
  } else {
    return util.promisify(options.theme.options.renderer || this.defaultRenderer).call(this, options.toRender, variables);
  }
};
