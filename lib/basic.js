...
const friendlyPromisify = function (promisifyTarget) {
  return function (...data) {
    return new Promise((resolve, reject) => {
      let resolved;

      const result = promisifyTarget.apply(this, data.concat((err, callbackData) => {
        if (resolved) { return; }
      ...
      if (result && result.then && !resolved) {
        resolved = true;
...
Tema.prototype.preprocess = function (template, variables, context) {
  ...
  this.themeTree.forEach(theme => {
    if (theme.preprocessor) {
      promiseChain = promiseChain.then(friendlyPromisify(theme.preprocessor.bind(this)));
    }
  });
  this.themeTree.forEach(theme => {
    if (theme.preprocessors && theme.preprocessors[template]) {
      promiseChain = promiseChain.then(data => data.variables)
        .then(friendlyPromisify(theme.preprocessors[template].bind(this)))
        .then(variables => ({ template: template, variables: variables }));
    }
  });
  this.themeTree.forEach(theme => {
    if (theme.processor) {
      promiseChain = promiseChain.then(friendlyPromisify(theme.processor.bind(this)));
    }
  });
  this.themeTree.forEach(theme => {
    if (theme.processors && theme.processors[template]) {
      promiseChain = promiseChain.then(data => data.variables)
        .then(friendlyPromisify(theme.processors[template].bind(this)))
