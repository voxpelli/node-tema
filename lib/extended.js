...
recursiveRenderer = function (element) {
  ...
  _.each(['preRenders', 'templateWrappers', 'children', 'postRenders'], function (property) {
    if (element[property]) {
  ...
  .then(function (content) {
      _.each(element.children, function (child) {
        _.merge(element.renderContext, child.renderContext || {}, function (destination, source) {
  ...
  .then(function (content) {
      ...
      _.each(element.templateWrappers || [], function (templateWrapper) {
        ...
        contentPromise = contentPromise.then(function (content) {
          input = content;
          return this.renderPromise(templateWrapper, _.extend({}, element.variables || {}, {
            element : element,
            content : content
          }), element.renderContext)
          // Below ensures that a missing template amongst the wrappers will fail silently
          .then(function (content) { return content; }, function (err) {
            console.warn(err);
            return input;
          });
        }.bind(this));
...
Extended = Tema.extend({
  ...
  recursiveRenderer : Promise.nodeify(recursiveRenderer)
});
